...
public abstract class AbstractRoomGenerator extends AbstractGenerator {
    ...
    public static class Room {
        ...
        public void show(final Grid grid) {
            for (int x = this.x, sizeX = this.x + width; x < sizeX; x++) {
                for (int y = this.y, sizeY = this.y + height; y < sizeY; y++) {
                    grid.setVisibility(x, y, true);
                }
            }
        }

        public void setType(final Grid grid){
            for (int x = this.x, sizeX = this.x + width; x < sizeX; x++) {
                for (int y = this.y, sizeY = this.y + height; y < sizeY; y++) {
        ...
        public void setDevice(final Device device){
            this.device = device;
            this.type = DeviceSystem.getIndex(device.getClass());
        }
        ...
        public boolean isNeighbor(DungeonGenerator.Point point) {
            return this.x==point.x+1 || this.y==point.y+1 || this.x+width==point.x || this.y+height==point.y;
        }
